...
void scan_key_matrix()
{
 ...
 for(i=0;i<4;i++)
 {
  PORTD = ~arrayMaskOutputOfKey[i];
  for(j=0;j<4;j++)
  {
   if((PORTD & arrayMaskInputOfKey[j]) == 0)
    key_code[i*4 +j] = key_code[i*4 +j] + 1;
   else
    key_code[i*4 +j] = 0;
  }
 }
...
void scan_key_button()
{
 ...
 for(i=0;i<8;i++)
 {
        if((PORTD & arrayMaskInputOfButton[i]) == 0)
